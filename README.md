...
src/
  core/        # 순수 게임 로직 (렌더링 무관)
    board.ts       # 보드 생성, 리롤, 사각형 합 판정, 조합 고갈 검사
    scoring.ts     # 점수 규칙
    game.ts        # 타이머, 상태 머신(ready → playing → result), 리롤 자원
  ui/
    renderer.ts    # 캔버스 렌더링 (타일, 선택 영역, 합계 배지, 점수 플로팅)
    input.ts       # 포인터(터치/마우스) 드래그 입력
  main.ts      # DOM/HUD 연결, 게임 루프, 로컬 최고 기록
```
...
⬜ M2 — 코인 환산, 스킬트리, 로컬 저장 확장
⬜ M3 — 특수 조합(13/17/20), 콤보, 연출
